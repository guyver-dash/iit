...
<script>
  ...
  export default {
      ...
      methods: {
        changePage(){
          let data = this
          this.$http.get(base_api + '/ict?page=' + this.page + '&token=' + window.localStorage.getItem('tokenKey')
          )
          .then(function(res){
      ...
      watch: {
        search: function(val){
          var data = this
          this.$http.post(base_api + '/ict/search?token=' + window.localStorage.getItem('tokenKey'),{
              search: val
          })
            .then(function(res){
              data.$store.dispatch('enrollees', res.data.enrollees)
